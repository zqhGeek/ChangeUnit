...
public class Fragment_area extends Fragment implements OnItemSelectedListener {
	private View view;
	...
	public View onCreateView(LayoutInflater inflater,
		...
		view = inflater.inflate(R.layout.area, container, false);
		et = (EditText) view.findViewById(R.id.et_input_area);
		et.setInputType(InputType.TYPE_NULL);// 不弹出输入法
		mySpinner_top = (Spinner) view.findViewById(R.id.spinner_top_area);
		mySpinner = (Spinner) view.findViewById(R.id.spinner_area);
		tv = (TextView) view.findViewById(R.id.tv_result_area);
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals("zqh.number.0")) {
				...
				if (result.equals("")) {
					...
					float output = changeUnit.ChangetoUnitarea(unit, result,
							unit2);
